...
public class JuliaSet extends Render{
	private int depth, width, height;
	private float pXV, pYV;
	private float distanceX, distanceY;
	private ArrayList<Point> points;
	
	public JuliaSet(int depth, int width, int height) {
		super(width, height);
		this.depth = depth;
		this.width = width;
		this.height = height;
	...
	public void zoom(float cx, float cy, float s, Complex c){
		long a = System.currentTimeMillis();
		//System.out.println("1 zoom:" + s);
		generate(cx - s, cx + s, cy - s, cy + s, c);
		//System.out.println("2 time:" + (float)(System.currentTimeMillis() - a)/1000f);
	}
	
	public void generate(float minX,float maxX, float minY, float maxY, Complex c) {
		points.clear();
